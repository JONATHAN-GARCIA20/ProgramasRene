...
class Program
{
    static void Main()
    {
        ...
        // Definimos el array a ordenar
        int[] array = { 5, 3, 8, 1, 2 };

        // Llamamos a la función de ordenamiento por selección
        SelectionSort(array);

        // Imprimimos el mensaje de array ordenado
    ...
    // Método para ordenar el array usando el método de selección
    static void SelectionSort(int[] array)
    {
        int n = array.Length;
        for (int i = 0; i < n - 1; i++)
        {
            // Suponemos que el primer elemento de la sublista es el mínimo
            int minIndex = i;

            // Buscamos el índice del elemento mínimo en la sublista
            for (int j = i + 1; j < n; j++)
            {
