...
class Program
{
    static void Main()
    {
        try
        {
            // Solicitar el primer número al usuario (dividendo)
            Console.Write("Ingrese el primer numero (dividendo): ");
            int dividend = Convert.ToInt32(Console.ReadLine());

            // Solicitar el segundo número al usuario (divisor)
            Console.Write("Ingrese el segundo numero (divisor): ");
        ...
        {
            Console.WriteLine("Error: Entrada no válida, por favor ingrese números enteros.");
        }
    }
}

*/
// Programa en ARM64 Assembly para RaspbianOS que divide dos números
...
.data
    prompt1:      .asciz "Ingrese el primer numero (dividendo): " // Mensaje para solicitar el primer número
    prompt2:      .asciz "Ingrese el segundo numero (divisor): "  // Mensaje para solicitar el segundo número
    resultMsg:    .asciz "Resultado de la division: "             // Mensaje para mostrar el resultado
    errorMsg:     .asciz "Error: Division por cero no permitida\n" // Mensaje de error para división por cero
    newline:      .asciz "\n"                                     // Nueva línea
...
_start:
    // Mostrar el mensaje para solicitar el primer número (dividendo)
    ...
    mov w4, w0                      // Almacenar el primer número (dividendo) en w4

    // Mostrar el mensaje para solicitar el segundo número (divisor)
    mov x0, #1                      // Descriptor de archivo para STDOUT
    ldr x1, =prompt2                // Dirección del mensaje "Ingrese el segundo numero"
    mov x2, #36                     // Longitud del mensaje
    mov x8, #64                     // Syscall para 'write' (64)
    svc #0                          // Ejecutar syscall
    ...
    ldr w1, =buffer                 // Dirección del buffer
    bl atoi                         // Llamar a la función atoi para conversión
    mov w5, w0                      // Almacenar el segundo número (divisor) en w5

    // Verificar si el divisor es cero
    cbz w5, division_error          // Si el divisor es cero, saltar a la etiqueta de error

    // Realizar la división (primer número / segundo número)
    udiv w6, w4, w5                 // Divide w4 / w5 y almacena el resultado en w6
...
division_error:
    // Mostrar mensaje de error "Error: Division por cero no permitida"
    mov x0, #1                      // Descriptor de archivo para STDOUT
    ldr x1, =errorMsg               // Dirección del mensaje de error
